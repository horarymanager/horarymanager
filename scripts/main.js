...
function formulateDate(date){
  let finalDate = new Date(date?date:new Date());
...
router.get(['/', '', ' '], () => {
  ...
  let hoy = aguaTomadaHoy[formulateDate(new Date())]?aguaTomadaHoy[formulateDate(new Date())]:{
    count: [],
    total: 0
  };
  ...
  return `<div class="container">
    ...
    <hr>
    <div class="container-recordatorio-agua card">
      <div class="recordatorio-agua card-body bold">
        ${hoy.total==0?"Empieza ya a hidratarte.":(hoy.total<5?`Hoy has tomado un total de: ${hoy.total} vasos de agua. Recuerda mantenerte hidratado.`:`Solo te faltan ${8 - hoy.total} vasos de agua para cumplir la meta diaria.`)}
        <br><br>
        <button class="btn btn-success">Agregar Registro</button>
        <button class="btn btn-danger">Empezar Otra Vez</button>
      </div>
    </div>
    <br><br><br><br>
    <h5>Tareas de hoy</h5>
